...
var globalLogger = &Logger{output: log.New(os.Stdout, "", log.Lmsgprefix)}

// Переопределение глобального логгера
func SetupLogger(conf *config.Config) {
	...
	globalLogger = &Logger{output: l, level: ParseLevel(conf.LogLevel)}
}
